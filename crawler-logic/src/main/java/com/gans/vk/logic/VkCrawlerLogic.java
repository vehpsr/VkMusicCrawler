...
public class VkCrawlerLogic {
    ...
    private List<String> formatOutputData(List<Entry<String, Double>> aggregatedData, Multimap<String, Entry<AudioProcessor, Number>> metrics) {
        ...
        result.addAll(top(50, aggregatedData, metrics));
        ...
        boolean includeAll = new Boolean(SystemProperties.get(Property.CRAWLER_AUDIO_REPORT_INCLUDE_ALL_STATISTICS, "false"));
        if (includeAll) {
            result.add("\n\n\nAll aggregated data");
            Collections.reverse(aggregatedData);
            for (Entry<String, Double> entry : aggregatedData) {
                result.add(entry.getKey() + "\t" + FORMATER.format(entry.getValue()));
            }
        }
        return result;
    }

    private Collection<? extends String> top(int topCount, List<Entry<String, Double>> aggregatedData, Multimap<String, Entry<AudioProcessor, Number>> metrics) {
        List<String> result = new LinkedList<String>();
